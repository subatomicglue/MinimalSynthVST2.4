...
#if 1//WIN32
   // always used, win32 and mac
	#include "resource.h"
#else
enum
{
   // not used... (bmps start at 138, not 128, wtf).
	IDB_cknob = 128,
	IDB_vonoff,
	IDB_dbbrown,
	IDB_dbwhite,
...
void Editor::idle()
{
	...
	if(justloaded && ctrl_display)
	{
		...
		{
			ctrl_display->setline((float)ticks/1700);
		}
...
// initialize control array
	...
	CFrame* frame = new CFrame (rct, ptr, this);
...
void Editor::valueChanged (CDrawContext *pContext, CControl *control)
{
	if(!control)
		return;
...
//		ctrl_display->setDirty(true);
	...
	else if(index==n_display)
	{
		...
		sprintf(t,"Rumpelrausch ZR-3 V%s  P%02d",VERSION,curProgram);
