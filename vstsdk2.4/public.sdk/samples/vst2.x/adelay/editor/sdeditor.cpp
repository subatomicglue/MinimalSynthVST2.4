...
void squashStringConvert (float value, char* string)
{
	 sprintf (string, "%d", (int)(value * 30 + 1));
}
void floatStringConvert (float value, char* string)
{
	 sprintf (string, "%.01f", (value * 3.0f));
}
...
bool SDEditor::open (void *ptr)
{
    ...
    CBitmap* hKnobBody   = new CBitmap (kKnobBodyId);
    ...
    // knobby
    size (kKnobX, kKnobY,
          kKnobX + hKnobBody->getWidth (), kKnobY + hKnobBody->getWidth ());
    knobbyknob    = new CAnimKnob ( size, this, kKnob, 
                                    hKnobBody->getHeight () / hKnobBody->getWidth () /*num frames*/, 
                                    hKnobBody->getWidth () /* frame height */, hKnobBody, point );
	knobbyknob->setValue (effect->getParameter (kKnob));
	lFrame->addView (knobbyknob);

	// Power
	size (kFaderX, kFaderY,
          kFaderX + hFaderBody->getWidth (), kFaderY + hFaderBody->getHeight ());
	delayFader = new CVerticalSlider (size, this, kPower, minPos, maxPos, hFaderHandle, hFaderBody, point);
	delayFader->setOffsetHandle (offset);
	delayFader->setValue (effect->getParameter (kPower));
	lFrame->addView (delayFader);

	// FeedBack
	size.offset (kFaderInc + hFaderBody->getWidth (), 0);
	feedbackFader = new CVerticalSlider (size, this, kMorph, minPos, maxPos, hFaderHandle, hFaderBody, point);
	feedbackFader->setOffsetHandle (offset);
	feedbackFader->setValue (effect->getParameter (kMorph));
	lFrame->addView (feedbackFader);

	// Volume
	size.offset (kFaderInc + hFaderBody->getWidth (), 0);
	volumeFader = new CVerticalSlider (size, this, kGain, minPos, maxPos, hFaderHandle, hFaderBody, point);
	volumeFader->setOffsetHandle (offset);
	volumeFader->setValue (effect->getParameter (kGain));
	...
	delayDisplay->setValue (effect->getParameter (kPower));
	delayDisplay->setStringConvert (squashStringConvert);
	...
	feedbackDisplay->setValue (effect->getParameter (kMorph));
	...
	volumeDisplay->setValue (effect->getParameter (kGain));
	volumeDisplay->setStringConvert (floatStringConvert);
...
void SDEditor::setParameter (VstInt32 index, float value)
{
	...
	switch (index)
	{
		case kKnob:
		    if (knobbyknob)
				knobbyknob->setValue (effect->getParameter (index));
			break;
        case kPower:
			if (delayFader)
