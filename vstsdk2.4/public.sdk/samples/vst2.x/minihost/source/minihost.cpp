...
static const VstInt32 kNumProcessCycles = 100;
...
int main (int argc, char* argv[])
{
    ...
    ev.numEvents = 1;
    ev.events[0] = &e;
    ev.events[0]->type = kVstMidiType;
    VstMidiEvent* event = (VstMidiEvent*)ev.events[0];
    midiData = event->midiData;
    midiData[0] = (char)0x90; // note on
    midiData[1] = (char)60; // note
    midiData[2] = (char)126; // velocity
    //((AudioEffectX*)effect->object)->processEvents(&ev);
...
void checkEffectProcessing (AEffect* effect)
{
	...
	for (VstInt32 processCount = 0; processCount < kNumProcessCycles; processCount++)
	{
        ...
        if (processCount == 20)
        {
            midiData[0] = (char)0xb0; // all notes off
            midiData[1] = (char)0x7e; // all notes off
            ((AudioEffectX*)effect->object)->processEvents(&ev);
	    }
    }
